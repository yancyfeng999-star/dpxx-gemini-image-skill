...
DEFAULT_MODEL = GEMINI_31_FLASH_MODEL
...
SUPPORTED_PROFILES = (PROFILE_GEMINI,)
...
def add_profile_arguments(parser) -> None:
    parser.add_argument(
        ...
        default=None,
        help=("Credential profile. Omit for the Gemini default. "
...
def resolve_profile(profile: str | None, model: str | None) -> str:
    ...
    if model:
        raise SystemExit(f"Unsupported Gemini model: {model}")
    return PROFILE_GEMINI
...
def get_api_key(explicit_api_key: str | None, profile: str | None = None,
    ...
    raise SystemExit("Missing API key. Use --api-key or configure ROOTFLOWAI_GEMINI_API_KEY.")


def validate_size_for_model(size: str, model: str) -> None:
    """Gemini models accept all ratio sizes supported by this skill."""
    if model not in MODEL_PROFILE_MAP:
        raise SystemExit(f"Unsupported Gemini model: {model}")
